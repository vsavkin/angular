...
/**
 * @whatItDoes Configures if the scroll position needs to be restored when navigating back.
 *
 ...
 * @experimental
 */
export type ScrollPositionRestoration = 'disabled' | 'enabled' | 'top';

/**
 * @whatItDoes Configures if the router should scroll to the element when the url has a fragment.
 *
 * * 'disabled'--does nothing (default).
 * * 'enabled'--scrolls to the element.
 * @experimental
 */
export type AnchorScrolling = 'disabled' | 'enabled';
...
export class RouterScroller implements OnDestroy {
  ...
  init(): void {
    ...
    if (this.options.scrollPositionRestoration !== 'disabled') {
      this.scrollService.disableAutomaticScrollRestoration();
    }
  ...
  private createScrollEvents() {
    return this.router.events.subscribe(e => {
      if (e instanceof NavigationStart) {
        ...
        this.store[this.lastId] = this.scrollService.currentScrollPosition();
