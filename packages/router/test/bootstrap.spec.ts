...
import {APP_BASE_HREF, Location, ScrollService} from '@angular/common';
...
describe('bootstrap', () => {
  ...
  it('should restore the scrolling position', async(done) => {
    ...
    location.back();  // going back to /aa
    await waitForNavigationToComplete(router);
    expect(window.scrollY).toEqual(5000);

    location.forward();  // going "back" to /bb
    await waitForNavigationToComplete(router);
  ...
  function waitForNavigationToComplete(router: Router): Promise<any> {
    return toPromise.call(
        first.call(filter.call(router.events, (e: any) => e instanceof NavigationEnd)));
  }
});
